...
| [`guide_macos_arm64.md`](guide_macos_arm64.md) | 完整刷机指南（含拆机串口后路、故障排查）；host 端示例以 macOS 为主 |
...
| [`hardware_notes.md`](hardware_notes.md) | 射频能力/功率天花板/监管域/overlay whiteout 等实测笔记 |
| [`NAND-FORENSICS.md`](NAND-FORENSICS.md) | 原厂 NAND 逆向：代号映射、RSA 签名锁、OpenWrt 血统 |
| [`TOOLCHAIN_SHA256.md`](TOOLCHAIN_SHA256.md) | 全部文件的下载地址 + sha256 校验 |
...
> 工具链二进制（u-boot / initramfs / 固件）不入库，按 `TOOLCHAIN_SHA256.md` 自行下载校验。
...
2. **u-boot 请求的 .itb 文件名不带 `meraki_mr42-`** —— 名字不匹配 → TFTP 404 → 白灯不亮。下载时按 `TOOLCHAIN_SHA256.md` 用正确文件名（或建个别名）。
